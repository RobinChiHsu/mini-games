App.registerGame('tetris', ({ canvas, area, controls, onGameOver, onScore }) => {
  ...
  // Piece shapes: 7 types × 4 rotations × 4 cells [row, col]
  const SHAPES = {
  ...
  let grid, current, holdType, holdUsed, bag, nextQueue;
  ...
  function lockPiece() {
    ...
    // Check lock out (all cells in buffer)
    ...
    if (fullRows.length > 0) {
      ...
      score += (LINE_SCORES[fullRows.length] || 0) * level;
  ...
  function updateScore() {
    onScore(`${I18n.t('score')}: ${score}  LV${level}`);
  }
  ...
  function update(now) {
    if (gameOverFlag) {
      ...
      onGameOver({ score, extra: `LV${level} / ${lines} ${I18n.t('score') === '分數' ? '行' : 'lines'}` });
      return;
    }

    if (flashRows.length > 0) return; // Animating clear
  ...
  function draw(now) {
    ...
    // Level/Lines info
    const infoY = H - cellSize * 3;
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ctx.fillText(`LV ${level}`, 4, infoY);
    ctx.fillText(`${lines}`, 4, infoY + cellSize * 0.8);
  }
